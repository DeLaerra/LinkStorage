...
public class ReferenceController {
    ...
    public String addReferenceToUserHome(ReferenceDescription reference,
                                         @CurrentUser User user,
                                         @PathVariable Long refId) {
        log.info("Получен запрос на копирование записи ссылки в userHome: \n userId - {}, \n refDescriptionUid - {} ", user.getUid(), refId);
        referenceService.copyReference(refId, user, reference);
        return "redirect:/userHome";
    }
    ...
    public String sendReferenceToFriend(PrivateMessage privateMessage, Model model,
        ...
        privateMessageService.sendReferenceToFriend(privateMessage, refId, user, friendUsername, text);
    ...
    public String sendNewReferenceToFriend(PrivateMessage privateMessage, ReferenceDescription referenceDescription, Model model,
        ...
        referenceDescription = referenceService.addReference(0L, referenceDescription, url);
        privateMessage = privateMessageService.sendReferenceToFriend(privateMessage, referenceDescription.getUid(), user, friendUsername, text);
        log.info("Отправлена новая ссылка: \n username - {}, \n refDescriptionUid - {}, \n friendUsername - {} ",
                user.getUsername(), privateMessage.getReferenceDescription().getUid(), friendUsername);
        return "redirect:/userHome";
    }
}